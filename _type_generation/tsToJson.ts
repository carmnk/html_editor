...
function addSimpleType(def: Definition, type: JSONSchema7TypeName): boolean {
  for (const k in def) {
    if (!(simpleTypesAllowedProperties as any)[k]) {
      return false
    }
  }
...
export class JsonSchemaGenerator {
  ...
  private parseCommentsIntoDefinition(
    ...
    otherAnnotations: { [key: string]: any }
  ): void {
    if (!symbol) {
      return
    }
    ...
    jsdocs.forEach((doc) => {
      ...
      if (name.includes('.')) {
        ...
        if (parts.length === 2 && (subDefinitions as any)[parts[0]]) {
          ;(definition as any)[parts[0]] = {
            ...(definition as any)[parts[0]],
      ...
      if (
        (validationKeywords as any)[name] ||
  ...
  private getDefinitionForRootType(
    ...
    if (tupleType) {
      ...
      if (flags & ts.TypeFlags.String) {
        ...
        propertyTypeString.trim() === 'bigint' ||
        (definition as any).type === 'bigint'
      ) {
        definition.type = 'integer'
        ...
        if (value !== undefined) {
          ...
          // console.log("HEY HO HO", definition.type)
  ...
  private getDefinitionForProperty(
    ...
    if (valDecl?.initializer) {
      ...
      if ((<any>initial).expression) {
        ...
        (<any>initial).kind &&
        (<any>initial).kind === ts.SyntaxKind.NoSubstitutionTemplateLiteral
      ) {
  ...
  private getEnumDefinition(
    ...
    if (enumTypes.length) {
      console.log()
      ;(definition as any).type = 'any' // enumTypes.length === 1 ? enumTypes[0] : enumTypes;
    }
    ...
    if (schemas.length === 1) {
      for (const k in schemas[0]) {
        if (schemas[0].hasOwnProperty(k)) {
          ...
          const a = 1
          ;(definition as any)[k] = (schemas as any)[0][k]
        }
      }
    } else {
      const a = 1
      ;(definition as any)[unionModifier] = schemas
    }
    return definition
  }
  ...
  private getIntersectionDefinition(
    ...
    if (schemas.length === 1) {
      for (const k in schemas[0]) {
        if (schemas[0].hasOwnProperty(k)) {
          definition[k] = (schemas as any)[0][k] as any
        }
      }
    } else {
    ...
    if (
      ...
      if (clazz.members) {
        ...
        if (indexSignatures.length === 1) {
          ...
          const indexSymbol: ts.Symbol = (<any>indexSignature.parameters[0])
            .symbol
          ...
          if (typ.flags & ts.TypeFlags.IndexedAccess) {
            const targetName = ts.escapeLeadingUnderscores(
              (<any>clazzType).mapper?.target?.value
            )
            const indexedAccessType = <ts.IndexedAccessType>typ
            const symbols: Map<ts.__String, ts.Symbol> = (<any>(
      ...
      const propertyDefinitions = props.reduce((all: any, prop: any) => {
        const propertyName = prop.getName()
  ...
  public getMainFileSymbols(
    ...
    function includeFile(file: ts.SourceFile): boolean {
      ...
      return !!onlyIncludeFiles?.length //.filter((f) => pathEqual(f, file.fileName)).length > 0
    }
...
export function getProgramFromFiles(
  ...
  for (const k in compilerOptions) {
    if ((compilerOptions as any)?.hasOwnProperty(k)) {
...
export function buildGenerator(
  ...
  const settings = getDefaultArgs() as any
...
export function generateSchema(
  ...
  if (fullTypeName === '*') {
    ...
    return generator.getSchemaForSymbols(
      generator.getMainFileSymbols(program, onlyIncludeFiles)
    )
...
export async function exec(
  ...
  if (REGEX_TSCONFIG_NAME.test(path.basename(filePattern))) {
    ...
    program = getProgramFromFiles(onlyIncludeFiles as any, {
  ...
  if (args.out) {
    return new Promise((resolve, reject) => {
      fs.mkdir(path.dirname(args.out), { recursive: true }, function (
        mkErr: Error
      ) {
        if (mkErr) {
          return reject(
